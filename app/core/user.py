...
async def get_user_db(session: AsyncSession = Depends(get_async_session)):
    """Асинхронный генератор get_user_db.
    Он обеспечивает доступ к БД через SQLAlchemy и используется
...
class UserManager(IntegerIDMixin, BaseUserManager[User, int]):
    ...
    async def on_after_register(
        ...
        """Вывод информации после успешной регистрации пользователя.
        Например, можно настроить отправку письма на Email"""
